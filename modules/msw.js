...
module.exports=async (keyword, clientPostgres, browser)=>{
	...
	return new Promise((resolve, reject)=>{
		clientPostgres.query('SELECT location, imgur, count, created_at FROM mswImgur where location=\''+locationKey+'\';', async (err, res)=>{
			//console.log(err, res);
			if(err) throw err;
			...
			if(res.rows.length>0){//如果有截圖location
				...
				//先update查詢次數
				clientPostgres.query('UPDATE mswImgur SET count='+(row.count+1)+' WHERE location=\''+locationKey+'\';');
				//先update查詢次數
