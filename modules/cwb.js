...
const screenshotCwb=require("./screenshotCwb");

module.exports=async (keyword, clientPostgres, browser)=>{
	keyword=keyword.toUpperCase().replace("CWB", "").replace("C", "");
	...
	let locationKey=location;//要存postgres的key

	//如果有截圖locationKey
		//如果時間<=一小時，直接取用
			//tableName=cwbImgur, columnName=id, location, imgur, created_at
	...
	return new Promise((resolve, reject)=>{
		clientPostgres.query('SELECT location, imgur, count, created_at FROM cwbImgur where location=\''+locationKey+'\';', async (err, res)=>{
			//console.log(err, res);
			if(err) throw err;
			...
			if(res.rows.length>0){//如果有截圖location
				...
				//先update查詢次數
				clientPostgres.query('UPDATE cwbImgur SET count='+(row.count+1)+' WHERE location=\''+locationKey+'\';');
				//先update查詢次數
				...
				if(hours<=1){//如果時間<=一小時，直接取用
					...
					if(isAllLinksWork===false){
						...
						let imageBuffers=await screenshotCwb(url, browser, location);//截圖cwb+天數波浪預報
						...
						if(typeof imageBuffers==="undefined"){
							...
							console.log("有截圖cache但網址有問題，成功截圖："+location);
							...
							//update table
							clientPostgres.query('UPDATE cwbImgur SET imgur=\''+JSON.stringify(imageLinks)+'\', created_at=to_timestamp('+Date.now()+'/1000) WHERE location=\''+locationKey+'\';');
						}
					}
				}
				else{
					...
					if(typeof imageBuffers==="undefined"){
						...
						console.log("有截圖cache但時間超過，成功截圖："+location);
				...
				if(typeof imageBuffers==="undefined"){
					...
					console.log("沒有截圖cache，成功截圖"+location);
					...
					//insert table
					clientPostgres.query('INSERT INTO cwbImgur(location, imgur) VALUES (\''+locationKey+'\', \''+JSON.stringify(imageLinks)+'\');');
				}
			}
